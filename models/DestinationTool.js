...
const destinationToolSchema = new mongoose.Schema(
  {
...
destinationToolSchema.pre("validate", function (next) {
...
// Ensure title & slug uniqueness
destinationToolSchema.pre("save", async function (next) {
  try {
...
// Indexes
destinationToolSchema.index({ title: 1 }, { unique: true });
destinationToolSchema.index({ slug: 1 }, { unique: true });

module.exports = mongoose.model("DestinationTool", destinationToolSchema);
