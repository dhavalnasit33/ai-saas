...
Strictly follow this word count. Respond fully according to the word count.`;
      ...
      // Save user message asynchronously without blocking stream connection
      ChatService.addUserMessage(chat._id, userId, messageToSend).catch((err) =>
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // 4. Generate image using native Nano Banana (Google Gemini) API
      const imageBuffer = await aiService.generateNanoBananaEdit(
        prompt,
        imageFile,
        imageFile2,
      );

      // 5. Deduct 10 credits upon success
      await User.findByIdAndUpdate(req.user.id, {
        $inc: { image_credits: -creditCost },
      });

      // 6. Send binary image back
      res.set("Content-Type", "image/png");
