    ...
    try {
      ...
      switch (model) {
        ...
        case "ultra":
          ...
          break;
        case "stable-fast-3d":
          imageData = await aiService.generateStableFast3D(prompt, imageFile, user._id);
          contentType = "model/gltf-binary";
          break;
        case "sketch-to-image":
          imageData = await aiService.generateSketchToImage(prompt, imageFile, user._id);
...
router.post(
  ...
  async (req, res) => {
    ...
    const imageFile2 = req.files && req.files["image2"] ? req.files["image2"][0] : null;

    if (!imageFile) {
      return res.status(400).json({ success: false, error: "An input image is required for this tool." });
    }

    if (!prompt || typeof prompt !== "string" || prompt.trim() === "") {
      return res.status(400).json({ success: false, error: "A prompt/instruction is required for editing." });
    }
    ...
    try {
      ...
      if (!falApiKey || !falUrl) {
        throw new Error("Missing Fal AI API credentials in environment variables.");
      }
      ...
      const responseContentType = imageDownloadResponse.headers["content-type"] || "image/png";
      ...
      if (error.response) {
        ...
        console.error("Full Fal AI Response Data:", JSON.stringify(rawErrorData, null, 2));
      } else {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user.plan === "basic" || user.plan === "standard" || user.plan === "lite") {
        return res.status(403).json({
          success: false,
          error: "Basic and Free users cannot generate videos. Please upgrade to a Pro or Pro Max plan.",
        });
      }
      ...
      if (user.subscription_status === "trialing") {
        return res.status(403).json({
          success: false,
          error: "Video generation is not available during the free trial. Please wait for your trial to end and your subscription to become active.",
        });
      }

      // 2. Billing rate (9 credits/sec)
      const durationSeconds = parseInt(duration) || 3;
      ...
      for (const file of files) {
        ...
        } else if (file.fieldname.startsWith("video") || file.fieldname === "background") {
      ...
      for (let attempt = 0; attempt < 60; attempt++) {
        ...
        if (statusCheck.data?.status === "COMPLETED") {
          ...
          throw new Error(`Fal AI generation failed: ${statusCheck.data?.error || "Unknown error"}`);
        }
        console.log(`Polling Gemini Omni status: ${statusCheck.data?.status}`);
      }

      if (!generationFinished || !falResult || !falResult.video || !falResult.video.url) {
        throw new Error("Gemini Omni video generation timed out or returned invalid output.");
      }
      ...
      const responseContentType = videoDownloadResponse.headers["content-type"] || "video/mp4";
      ...
      if (error.response) {
        ...
        console.error("Fal.ai error response data:", JSON.stringify(rawData, null, 2));
        if (rawData && rawData.detail) {
          const details = Array.isArray(rawData.detail) ? rawData.detail : [rawData.detail];
          const detailMsg = details.map(d => d.msg || d.message || JSON.stringify(d)).join("; ");
          userMessage = `Video generation failed: ${detailMsg}`;
        } else if (rawData && JSON.stringify(rawData).includes("content_policy_violation")) {
          userMessage = "Your video request could not be processed due to content safety guidelines. Please modify your prompt or references.";
        }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const falUrl = "https://queue.fal.run/fal-ai/veo3.1/fast/reference-to-video";
      ...
      for (let attempt = 0; attempt < 60; attempt++) {
        ...
        console.log(`Polling Veo 3.1 Fast Reference-to-Video status: ${statusCheck.data?.status}`);
      }

      if (!generationFinished || !falResult || !falResult.video || !falResult.video.url) {
        throw new Error("Veo 3.1 Fast video generation timed out or returned invalid output.");
      }
